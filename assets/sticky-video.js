...
window.stickyVideo = window.stickyVideo || {};
...
(function(){
    ...
    var DOM_video, DOM_videoTag, DOM_placeholder, DOM_closeButton, DOM_player,
        ...
        topOffsetTrigger,
        boundingClientRect,
        stickyVideoFinalHeight, stickyVideoFinalWidth, stickyVideoVerticalOffset, stickyVideoSideOffset,
    ...
    var playerHandlers = {
        ...
        wpVideo:{
            ...
            preTransitioning: function(){
                if( DOM_player ){ // do not remove, in certain cases mejs is not used
                    mejs.players[ DOM_player.id ].setPlayerSize(stickyVideoFinalWidth, stickyVideoFinalHeight);
        ...
        wistia:{ // https://wistia.com/doc/player-api
            ...
            preTransitioning: function(){
                if( DOM_player ){
                    DOM_player.videoHeight( stickyVideoFinalHeight );
                    DOM_player.videoWidth( stickyVideoFinalWidth );
                }
            },
            resetStickyness: _emptyFunction,
            preResetTransitioning: function(){
    ...
    function init(){ // Important: this must be run on window.load
        ...
        isMobileMQ = (currentWindowWidth <= window.stickyVideo.params['mobile-breakpoint']);
    ...
    function selectVideo(){
        ...
        if( typeof window.stickyVideo.params['forced-selector'] == 'string' ){

            DOM_video = document.querySelector( window.stickyVideo.params['forced-selector'] );
        ...
        if( window.stickyVideo.params['selecting-method'] === 'class' ){
    ...
    function checkStickyness(){
        ...
        if( !window.stickyVideo.params['enable-on-mobile'] && isMobileMQ ){ // Mobile check
        ...
        if(
            ...
            if( // Pause check
                !window.stickyVideo.params['enable-on-pause'] &&
    ...
    function applyStickyness(){
        ...
        stickyVideoFinalWidth = window.stickyVideo.params['video-width' + (isMobileMQ?'-mobile':'')];
        stickyVideoFinalHeight = (stickyVideoFinalWidth/originalOuterWidth)*originalOuterHeight;
        stickyVideoVerticalOffset = window.stickyVideo.params['position-vertical-offset' + (isMobileMQ?'-mobile':'')];
        stickyVideoSideOffset = window.stickyVideo.params['position-side-offset' + (isMobileMQ?'-mobile':'')];
        ...
        DOM_video.style.setProperty('z-index', '9999999', 'important');
        DOM_video.style.setProperty('margin', '0', 'important');

        DOM_video.style.setProperty('width', stickyVideoFinalWidth + 'px', 'important');
        DOM_video.style.setProperty('height', stickyVideoFinalHeight + 'px', 'important');
        DOM_video.style.setProperty(
            window.stickyVideo.params['video-position'].indexOf('top-') !== -1 ? 'top' : 'bottom',
            stickyVideoVerticalOffset + 'px', 'important'
        );
        DOM_video.style.setProperty(
            window.stickyVideo.params['video-position'].indexOf('top-') !== -1 ? 'bottom' : 'top',
            'auto', 'important'
        );
        DOM_video.style.setProperty(
            window.stickyVideo.params['video-position'].indexOf('-left') !== -1 ? 'left' : 'right',
            stickyVideoSideOffset + 'px', 'important'
        );
        DOM_video.style.setProperty(
            window.stickyVideo.params['video-position'].indexOf('-left') !== -1 ? 'right' : 'left',
            'auto', 'important'
        );
